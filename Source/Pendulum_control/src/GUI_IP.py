...
class AnimationPlot:
    ...
    def animate(self, i, timeList, dataList1, dataList2, ser, file):
        ser.write(b'g')                                     # Transmit the char 'g' to receive the Arduino data point
        arduinoData_string = ser.readline().decode('ascii') # Decode receive Arduino data as a formatted string

        try:
            current_time, arduinoData_float1, arduinoData_float2 = map(float, arduinoData_string.split(','))  # Convert to two floats
            ...
            file.write(f'{current_time},{arduinoData_float1},{arduinoData_float2}\n')
        ...
        max_data2 = max(dataList2) if dataList2 else float('-inf')  # Get the maximum value of the last 50 points in dataList1
        min_data2 = min(dataList2) if dataList2 else float('inf')   # Get the minimum value of the last 50 points in dataList1
        ...
        ax.legend(loc='upper right')                        # Add a legend to differentiate between the two datasets
...
with open('data_log.txt', 'w') as file:
    ...
    ani = animation.FuncAnimation(fig, realTimePlot.animate, frames=100, fargs=(timeList, dataList1, dataList2, ser, file), interval=100) 
